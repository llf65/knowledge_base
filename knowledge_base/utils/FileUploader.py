...
from zskscript.config.config import api_config
    ...
    def upload_file_to_wps(self, file_obj, item):
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger.info(f"文件 {file_name} 上传成功.")
                try:
                    response_data = response.json()
                    if response_data['code'] == '0' and 'fileId' in response_data['body']['bizParam']:
                        fileId = response_data['body']['bizParam']['fileId']
                        self.logger.info(f"文件ID: {fileId}")
                        return fileId
                    else:
                        self.logger.error(f"上传成功，但未找到响应中的fileId。响应内容: {response_data}")
                        return None
